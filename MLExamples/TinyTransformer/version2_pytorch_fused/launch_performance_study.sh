...
echo "  - QKV Fusion        - Combines 3 attention projections"
echo "  - Flash Attention   - Memory-efficient attention"
echo "  - SwiGLU Fusion     - Fused gate activation"
echo "  - Torch Compile     - JIT optimization"
echo ""
...
# Build command with all fusion optimizations enabled
CMD="python tiny_llama_v2.py \
...
# Run training
echo "Starting V2 Fused training with all optimizations..."
echo ""
...
# Check for V1 comparison
if grep -q "Speedup vs V1:" "$OUTPUT_DIR/training_output.log"; then
    SPEEDUP=$(grep "Actual Speedup:" "$OUTPUT_DIR/training_output.log" | tail -1 | awk '{print $3}')
    echo "Speedup vs V1: $SPEEDUP"
fi

# Check for optimization efficiency
if grep -q "Optimization Efficiency:" "$OUTPUT_DIR/training_output.log"; then
    EFFICIENCY=$(grep "Optimization Efficiency:" "$OUTPUT_DIR/training_output.log" | tail -1 | awk '{print $3}')
    echo "Optimization Efficiency: $EFFICIENCY"
fi

echo ""
echo "Results saved to: $OUTPUT_DIR/"
echo "  - config.json              Configuration metadata"
echo "  - training_output.log      Full training output with fusion analysis"
echo "  - performance_summary_v2.json  Performance metrics and comparisons"
if [ "$ENABLE_PROFILERS" = true ]; then
    echo "  - pytorch_profiles_v2/     PyTorch profiler data (if enabled)"
fi

echo ""
echo "Next Steps:"
echo "  1. Review fusion efficiency: grep -A10 'Optimization Strategy' $OUTPUT_DIR/training_output.log"
echo "  2. Compare to V1: grep -A5 'Comparison vs Version 1' $OUTPUT_DIR/training_output.log"
if [ "$ENABLE_PROFILERS" = true ]; then
    echo "  3. Analyze profiling: tensorboard --logdir $OUTPUT_DIR"
fi
echo "  4. Run V3 for comparison: cd ../version3_triton && ./launch_performance_study.sh $SIZE"
echo "  5. Compare all versions: python ../compare_versions.py"
echo ""
