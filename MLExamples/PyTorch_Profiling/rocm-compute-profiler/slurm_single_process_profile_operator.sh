#!/bin/bash
#SBATCH --job-name=rpc-single-profile-operator
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --gpus=1
#SBATCH --time=02:00:00
#SBATCH --output=rpc_single_process_profile_operator_%j.out
#SBATCH --error=rpc_single_process_profile_operator_%j.err

# ---------------------------------------------------------------------------
# Profile the single-process CIFAR-100 workload with rocprof-compute profile,
# also capturing a PyTorch operator trace (--torch-trace).
...
#  * --experimental --torch-trace additionally captures a PyTorch operator trace.
