#!/usr/bin/env python3
"""Load a trace into the real Perfetto UI (ui.perfetto.dev) with headless Google
Chrome and screenshot the timeline.  Works for any trace Perfetto can open:
rocprofv3 ``*.pftrace``, rocprofiler-systems ``*.proto``, and torch.profiler
``*.pt.trace.json`` (Kineto).  Uses Perfetto's official window.postMessage
deep-linking API (an opener page opens the UI in a popup, waits for the PING/PONG
handshake, then posts the trace ArrayBuffer) so there is no CORS / mixed-content /
local-server dependency.

Adapted from MPI-examples/cg-solver-example/CG-GPU/screenshot_perfetto.py; the
only FSDP2 change is the default search label (an RCCL collective kernel instead
of a roctx range).

Usage:
  python screenshot_perfetto.py <trace> <out.png> [title] [wait_ms] [label] [zoomout]
Requires: playwright (pip), a reachable ui.perfetto.dev, and google-chrome
(module load google-chrome/stable; export CHROME_BIN=$(command -v google-chrome)).
"""
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # Expand all track groups so the GPU-kernel + RCCL rows are visible.
        try:
            omnibox("Expand all", ">")
        ...
        # Search for a kernel/label -> Perfetto selects that slice and pans to it.
        # Escape leaves the omnibox (selection persists), 'f' zooms to it, then 's'
        # zooms back out a little to reveal the neighbouring steps.
        # argv: 5=label 6=zoom_out_presses.  Default: an FSDP2 RCCL collective.
        # RCCL 2.27 fuses collectives into ncclDevKernel_Generic_* (not _AllGather_*).
        label   = sys.argv[5] if len(sys.argv) > 5 else "ncclDevKernel_Generic"
        ...
        if label and label != "-":
            try:
                ...
                for _ in range(zoomout):              # widen to a couple of steps
                    ui.keyboard.press("s")
                    ui.wait_for_timeout(150)
                ui.wait_for_timeout(800)
            except Exception as e:
