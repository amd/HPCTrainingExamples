...
| [Score-P](../../common/profilers/scorep.md) | `module load scorep` + venv | Python regions | Per-rank training-loop regions, written to Cube/OTF2 |
...
language (ops, `nccl:all_reduce`, module names) and needs no extra tooling. We drop
to `rocprofv3`/`rocprof-compute` when we need kernel-level or roofline detail,
and to `rocprofiler-systems` when we need to *see* the overlap on a timeline.
...
## Ground rules for trustworthy numbers
...
- Nodes must be booted with `iommu=pt` so RCCL uses direct xGMI P2P
  ...
  the "comm" number measures the wrong thing.
- Warm up MIOpen first (`export MIOPEN_FIND_MODE=FAST`, or run `../benchmarks/warm_miopen.py`)
...
| Goal | Guide | Command |
|------|-------|---------|
| Op/kernel + RCCL table, trace | [torch.profiler](../../common/profilers/torch-profiler.md) | `torchrun ... ../benchmarks/ddp_resnet_bench.py --profile --profile-dir ./torch_prof` |
| Model FLOPs / MACs / params | [DeepSpeed](../../common/profilers/deepspeed-flops.md) | `torchrun ... ../benchmarks/ddp_resnet_bench.py --flops` |
...
| Per-rank Python regions | [Score-P](../../common/profilers/scorep.md) | `NPROC=2 ../../common/scorep_launch.sh ../benchmarks/ddp_resnet_bench.py ...` |
