...
## 2. Get an allocation and load PyTorch
...
> convolutions. Set fast selection, then warm the cache by running the **1-GPU
> `main.py` case** (the same run the sweep uses) for a few steps:
...
python -c "import torch,torchvision.models as M; \
   d=torch.device('cuda'); \
   n=M.resnet50().to(d); \
...
```bash
export HIP_VISIBLE_DEVICES=0
python main.py -a resnet50 --dummy --dist-url 'tcp://127.0.0.1:23456' \
        --dist-backend nccl --multiprocessing-distributed --world-size 1 --rank 0 -b 128  -p 20 --epochs 1
export HIP_VISIBLE_DEVICES=0,1
python main.py -a resnet50 --dummy --dist-url 'tcp://127.0.0.1:23456' \
        --dist-backend nccl --multiprocessing-distributed --world-size 1 --rank 0 -b 256  -p 20 --epochs 1
export HIP_VISIBLE_DEVICES=0,1,2,3
python main.py -a resnet50 --dummy --dist-url 'tcp://127.0.0.1:23456' \
        --dist-backend nccl --multiprocessing-distributed --world-size 1 --rank 0 -b 512  -p 20 --epochs 1
```
...
[`testit.sh`](testit.sh)) and prints one line per GPU count:
...
The [`testit.sh`](testit.sh) driver runs the sweep above **and** adds two extra
...
`testit.sh`); if the migrate extension can't build, `Stager` falls back to a copy
