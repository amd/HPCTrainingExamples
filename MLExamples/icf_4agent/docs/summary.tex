...
\title{Reinforcement Learning Optimization of ICF Capsule Designs via Adversarial Multi-Agent Systems and Reduced-Order Modeling}

\author{\IEEEauthorblockN{Jonathan L. Belof, Sidafa Conde, Nicholas Malaya, Harry Souris, Rangan Sukumar and Kelly Marquardt}
\IEEEauthorblockA{\textit{Advanced Micro Devices, Inc.}\\
...
The system connects to a single LLM served over an OpenAI-compatible endpoint, and the same client interface covers both on-premises and hosted deployments.  For fully local, security-sensitive work---where sending capsule parameters to an external service is undesirable---an open-weight model fine-tuned on HEDP literature (\texttt{gptoss-20b-hedp}, 20B parameters) is served through a high-throughput inference engine (vLLM) on AMD Instinct GPUs, keeping the entire reasoning--simulation loop on-premises.  For larger or shared deployments the identical client drives a \emph{remote} inference server with no code changes---in particular the \textbf{AMD Inference Microservice (AIMS)}, a containerized, ROCm-based, production-ready inference service (part of the AMD Enterprise AI Suite\footnote{\url{https://enterprise-ai.docs.amd.com/en/latest/aims/overview.html}}) that serves models on AMD Instinct GPUs and, like vLLM, exposes an OpenAI-compatible API.  Beyond that API, AIMS adds automatic hardware-profile selection, model caching, integrated health/metrics, and Kubernetes/KServe scaling, turning model serving into a largely one-command deployment; in our remote runs the swarm is served an \texttt{openai/gpt-oss-120b} model through an AIMS endpoint.  In all cases a single shared model backs every agent role; the architecture permits distinct per-role models (e.g.\ a domain-tuned Explorer alongside a frontier PI), but the reference implementation uses one model configured entirely through environment variables, keeping credentials out of the source tree.
...
These calculations were run on LUX, the first AI for Science Factory in the United States, sited at Oak Ridge National Laboratory (ORNL).  The authors thank the staff of ORNL and Lawrence Livermore National Laboratory---Charles Jekel, M. Giselle Fernandez-Godino, Will Schill, Harshitha Menon, and many others---for their guidance on simulation methods and agents.
...
\bibitem{shachar2025} M. H. Shachar \textit{et al.}, ``Multi-Agent Design Assistant for the Simulation of Inertial Fusion Energy,'' accepted for publication in \textit{Phys. Rev. E}, 2025. [Online]. Available: \url{https://arxiv.org/abs/2510.17830}
\end{thebibliography}
