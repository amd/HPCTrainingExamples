PROGRAM problem
  ...
  integer, parameter :: NI=448, NJ=448, NK=448, rk=8, maxrepeat=100
  ...
  do i_rb =1,2
    ...
    !$omp parallel do collapse(3) private(i,j,k) shared(f,i_rb)
    do k = 0,NK+1
      do j = 0,NJ+1
        do i = 0,NI/2
          ii = 2*i + mod(j+k+i_rb,2)
          f(ii,j,k) = real(i_rb, kind=rk) 
        end do
      end do
    end do
  end do
    ...
    do k = 1,NK
      do j = 1,NJ
        do i = 1,NI/2
          ...
          f(ii,j,k) = (f(ii-1,j,k) + f(ii+1,j,k) +      &
                       f(ii,j-1,k) + f(ii,j+1,k) +      &
                       f(ii,j,k-1) + f(ii,j,k+1) +      &
                       f(ii,j,k)) / 7.0_rk
        end do
      end do
    end do
