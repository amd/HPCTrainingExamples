...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (strcmp(argv[i], "--low-synch") == 0 && argc > i + 1) {
      low_synch = atoi(argv[++i]);
    }
  }
  ...
  printf("CG-CG (Chronopoulos/Gear) setup: \n");
  printf("\t Maxit:                                %d\n", maxit);
  ...
  // Create spmv descriptor and set inputs
  ...
  printf("Preconditioner ready, starting CG-CG ...\n\n");
  ...
  CGCGResult result = cg_cg_solve(A, d_x, d_b, maxit, tol,
  ...
  printf("\nCG-CG summary results \n");
  printf("\t Iters              : %d  \n", result.iterations);
  printf("\t Res. norm          : %2.16g  \n", result.final_residual);
  if (result.flag == 0) {
    printf("\t Reason for exiting : CG-CG converged  \n");
  } else if (result.flag == 1) {
    printf("\t Reason for exiting : CG-CG reached maxit \n");
  } else {
    printf("\t Reason for exiting : CG-CG failed\n");
  }
  printf("\t Time (ms)          : %.3f\n", elapsed_ms);
  printf("\n");
