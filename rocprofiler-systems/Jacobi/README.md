...
In this series of examples, we will demonstrate profiling with rocprof-sys on a platform using an AMD Instinct&trade; MI300 GPU, hosted in AAC environment. ROCm releases (6.3.2) include rocprof-sys.

Note that the focus of this exercise is on rocprof-sys profiler, not on how to achieve optimal performance on MI300A.
...
For this training, one requires recent ROCm (>=6.3) which contains rocprof-sys, as well as an MPI module.
