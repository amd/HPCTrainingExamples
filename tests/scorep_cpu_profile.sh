...
MAKEFILE_DIR=${REPO_DIR}/Pragma_Examples/OpenMP/Fortran/8_jacobi/0_jacobi_portyourself
...
cp ${SRC_DIR}/*.f90 ${BUILDDIR}/
cp ${MAKEFILE_DIR}/Makefile ${BUILDDIR}/
cd ${BUILDDIR}

echo "Building CPU Jacobi with Score-P instrumentation..."
...
echo "Running instrumented Jacobi (CPU, mesh 256)..."
./jacobi -m 256
if [ $? -ne 0 ]; then
...
echo "${SCORE_OUTPUT}" | grep -q '!\$omp'
if [ $? -ne 0 ]; then
   echo "FAIL: scorep-score output missing expected OpenMP regions"
   exit 1
fi

echo "PASS: Score-P CPU profiling test completed successfully"
