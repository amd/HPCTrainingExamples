#!/bin/bash
#
# Run the Fortran offloading subset of the OpenMP Offloading Validation &
# Verification (OMPVV / SOLLVE) test suite on an AMD MI300A (gfx942) GPU.
#
...
#
# This script compiles and runs only the Fortran tests (FC=amdflang) across all
# OpenMP spec versions (4.5, 5.0, 5.1, 5.2) using the amdflang compiler.
#
# Usage:
#   ./sollve_vv_fortran_offload.sh [ROCM_VERSION]
#
#   ROCM_VERSION  optional module version, e.g. 7.0.2 -> "module load rocm/7.0.2".
#                 If omitted, the currently loaded rocm module is used, or the
#                 default rocm module is loaded.
#
# Examples:
#   ./sollve_vv_fortran_offload.sh 7.0.2
#   ./sollve_vv_fortran_offload.sh            # use loaded / default rocm module
#
...
RESULTS_DIR="${SCRIPT_DIR}/results_fortran_${DATE_TAG}"
mkdir -p "${RESULTS_DIR}"
...
echo "==================================================================="
echo " OpenMP_VV Fortran offloading run on ${GFX_MODEL}"
...
for v in 4.5 5.0 5.1 5.2; do
   ...
   # CC=none CXX=none forces Fortran-only: the compiler modules export CC/CXX in
   # the environment, which would otherwise make the suite also build C/C++.
   make CC=none CXX=none FC=amdflang OMP_VERSION=${v} OMPV="${OMPV_FLAG}" BINDIR=bin_${v} LOGDIRNAME=logs_${v} LOG_ALL=1 all
