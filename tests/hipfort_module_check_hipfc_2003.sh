#!/bin/bash

if [[ "`printenv |grep -w CRAY |wc -l`" -gt 1 ]]; then
   if [ -z "$CXX" ]; then
      export CXX=`which CC`
   fi
   if [ -z "$CC" ]; then
      export CC=`which cc`
   fi
   if [ -z "$FC" ]; then
      export FC=`which ftn`
   fi
else
   module list 2>&1 | grep -q -w "rocm"
   if [ $? -eq 1 ]; then
     echo "rocm module is not loaded"
     echo "loading default rocm module"
     module load rocm
   fi
   module load amdflang-new >& /dev/null
   if [ "$?" == "1" ]; then
      module load amdclang
   fi
fi

module load hipfort_from_source
AMDGPU_GFXMODEL=`rocminfo | grep gfx | sed -e 's/Name://' | head -1 |sed 's/ //g'`

git clone https://github.com/ROCm/hipfort hipfort_for_test_module_2003

pushd hipfort_for_test_module_2003/test/f2003/vecadd

HIPFORT_COMP=`which amdflang`

# Try example from source director
hipfc -v --offload-arch=${AMDGPU_GFXMODEL} -hipfort-compiler $HIPFORT_COMP  hip_implementation.cpp main.f03
./a.out

popd

rm -rf hipfort_for_test_module_2003

module unload hipfort_from_source

