...
if [ -n "${CRAY_MPICH_VERSION:-}" ]; then
   ...
   MPI_RESOURCE_MPI4="--bind-to core --map-by ppr:${NUM_PER_RESOURCE_MPI4}:numa --report-bindings"
   MPI_RESOURCE_MPI16="--bind-to core --map-by ppr:${NUM_PER_RESOURCE_MPI16}:numa --report-bindings"
fi
