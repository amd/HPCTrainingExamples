...
if ! module load rccl-tests/${ROCM_VER} 2>/dev/null; then
...
OUT=$(timeout 300 "${ALL_REDUCE}" -b 8M -e 512M -f 2 -g "${GPU_COUNT}" -c 1 -n 50) || { echo "FAIL: all_reduce_perf run failed"; exit 1; }
