...
#pragma omp target enter data map(alloc: xbuf_left_send[:bufcount], xbuf_rght_send[:bufcount])
#pragma omp target enter data map(alloc: xbuf_rght_recv[:bufcount], xbuf_left_recv[:bufcount])
...
#pragma omp target exit data map(delete: xbuf_left_send, xbuf_rght_send, xbuf_rght_recv, xbuf_left_recv)
   free(xbuf_left_send);
   free(xbuf_rght_send);
   free(xbuf_rght_recv);
   free(xbuf_left_recv);
...
//#pragma omp target update from(x[-nhalo*2:(imax+nhalo)*(jmax+nhalo))
