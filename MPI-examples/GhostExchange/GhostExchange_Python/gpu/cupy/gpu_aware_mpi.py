...
def ghostcell_update(x, nhalo, corners, jsize, isize, nleft, nrght, nbot, ntop, do_timing):
    ...
    xbuf_left_send = cp.zeros(bufcount, dtype=np.float64)
    xbuf_rght_send = cp.zeros(bufcount, dtype=np.float64)
    xbuf_rght_recv = cp.zeros(bufcount, dtype=np.float64)
    xbuf_left_recv = cp.zeros(bufcount, dtype=np.float64)
    ...
    if corners:
        ...
        if ntop != MPI.PROC_NULL:
            recv_buf_top = cp.zeros((nhalo, isize + 2*nhalo), dtype=np.float64)
        ...
        if nbot != MPI.PROC_NULL:
            recv_buf_bot = cp.zeros((nhalo, isize + 2*nhalo), dtype=np.float64)
...
def main():
    ...
    x = cp.zeros((jsize_with_halos, isize_with_halos), dtype=np.float64)
    xnew = cp.zeros((jsize_with_halos, isize_with_halos), dtype=np.float64)

    # init soln
    i_indices, j_indices = cp.meshgrid(cp.arange(isize), cp.arange(jsize))
...
#    x=cp.asarray(x_cpu)
    ...
    if do_print:
       ...
       x_cpu=cp.asnumpy(x)
       write_netcdf_soln(x_cpu, jmax, imax, nhalo, nprocy, nprocx, 0, ncid, varid);
