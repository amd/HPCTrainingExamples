...
constexpr size_t partially_allocate_lds = 64ul * 1024ul / sizeof(double)/32;
...
__global__ void yax(double* y,
		    ...
		    int n, int m, 
		    double* result){
  __shared__ double tmp[partially_allocate_lds];
  for(int i = threadIdx.x; i < partially_allocate_lds; i+= blockDim.x) tmp[i] = x[i];
  __syncthreads();
  double res = 0.0;
  for(int i = blockDim.x * blockIdx.x + threadIdx.x; i < n; 
    ...
    double temp = 0.0;
    for(int j = 0; j < m; j++){
      temp += A[i*n+j] * (j < partially_allocate_lds ? tmp[j] : x[j]);
    }
    res += y[i] * temp;
  }
