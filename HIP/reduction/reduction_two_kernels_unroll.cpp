...
__global__ void remaining_sum(const double* input, double* output, int size) {
  extern __shared__ double local_sum[];
...
int main() {
  ...
  if (nblocks > 1) {
     ...
     hipCheck(hipMemcpy(h_in.data(), d_partial_sums, 1 * sizeof(double), hipMemcpyDeviceToHost));
  }
